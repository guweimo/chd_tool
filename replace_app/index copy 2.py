...
class FolderSelectorApp(QMainWindow):
    ...
    def create_function3_widget(self):
        ...
        # 目标配置下拉菜单
        self.target_default_combo = QComboBox()
        self.target_default_combo.setItemDelegate(StyledComboBoxDelegate(self.target_default_combo))
        self.target_default_combo.setPlaceholderText("选择目标配置")
        self.target_default_combo.setStyleSheet(self.select_style)
        layout.addRow("目标配置:", self.target_default_combo)
    ...
    def create_function4_widget(self):
        ...
        self.target_option_combo = QComboBox()
        self.target_option_combo.setItemDelegate(StyledComboBoxDelegate(self.target_option_combo))
        self.target_option_combo.setPlaceholderText("选择目标配置")
        self.target_option_combo.setStyleSheet(self.select_style)
    ...
    def update_option_config_combos(self):
        ...
        # 添加"所有配置"选项
        self.target_option_combo.addItem("所有配置", "all")
        
        for path, name in self.folders.items():
            self.source_option_combo.addItem(name, path)
            self.target_option_combo.addItem(name, path)
    ...
    def update_default_config_combos(self):
        ...
        for path, name in self.folders.items():
            ...
            self.target_default_combo.addItem(name, path)
        
        # 默认选中第一个
        if self.source_default_combo.count() > 0:
            self.source_default_combo.setCurrentIndex(0)
            if self.target_default_combo.count() > 1:
                self.target_default_combo.setCurrentIndex(1)
    ...
    def execute_function1(self):
        ...
        self.log_operation(f"执行: 在所有配置中替换装备 {current_equip} -> {replace_equip}")
            ...
            for path, name in self.folders.items():
                ...
                if "diysuit_item" in config_data:
                    ...
                    if updated:
                        ...
                        updated_files += 1
                        self.log_operation(f"成功: {name} 中替换了装备")
    ...
    def execute_function2(self):
        ...
        self.log_operation(f"执行: 从配置 {source_name} 复制装备配置 {target_config_name} 的data到其他配置")

        # 实际替换逻辑
        try:
            target_configs = [path for path in self.folders.keys() 
            ...
            for target_path in target_configs:
                ...
                if "diysuit_item" in target_data:
                    ...
                    if updated:
                        ...
                        updated_count += 1
                        self.log_operation(f"成功更新配置: {self.folders[target_path]}")
                    else:
                        self.log_operation(f"配置 {self.folders[target_path]} 中未找到名称匹配的装备")
                else:
                    self.log_operation(f"配置 {self.folders[target_path]} 中没有diysuit_item数据")
    ...
    def execute_function3(self):
        ...
        if self.target_default_combo.currentIndex() == -1:
            QMessageBox.warning(self, "警告", "请选择目标配置!")
            return
        
        source_path = self.source_default_combo.currentData()
        target_path = self.target_default_combo.currentData()
        ...
        target_name = self.target_default_combo.currentText()
        
        self.log_operation(f"执行: 从配置 {source_name} 复制Default.save到配置 {target_name}")
        ...
        try:
            ...
            self.log_operation(f"成功: 已将 {source_name} 的Default.save复制到 {target_name}")
            
            QMessageBox.information(self, "成功", f"已成功将 {source_name} 的Default.save复制到 {target_name}")
    ...
    def execute_function4(self):
        ...
        if self.target_option_combo.currentIndex() == -1:
            QMessageBox.warning(self, "警告", "请选择目标配置!")
            return
        ...
        target_path = self.target_option_combo.currentData()
