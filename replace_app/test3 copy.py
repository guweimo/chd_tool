...
class FolderSelectorApp(QMainWindow):
    ...
    def update_option_config_combos(self):
        ...
        # 连接信号，处理选择变化
        self.target_option_combo.model().itemChanged.connect(self.handle_target_selection_change)

        # 默认选中第一个源配置
        if self.source_option_combo.count() > 0:
            self.source_option_combo.setCurrentIndex(0)

    def handle_target_selection_change(self, item):
        """处理目标配置选择变化"""
        model = self.target_option_combo.model()
