...
class CheckableComboBox(QComboBox):
    def __init__(self, parent=None):
        ...
        # 初始显示文本
        self.setPlaceholderText("请选择...")
        
        # 连接信号，当项目变化时更新显示
        self.model().itemChanged.connect(self.updateDisplayText)
        
    def eventFilter(self, obj, event):
        # 处理鼠标释放事件，使整个选项区域都可点击
...
class FolderSelectorApp(QMainWindow):
    ...
    def execute_function3(self):
        ...
        if len(self.target_default_combo.checkedItems()) == 0:
        ...
        # 获取选中的目标配置
        target_paths = self.target_default_combo.checkedData()
        
        self.log_operation(f"执行: 从配置 {source_name} 复制Default.save到选中的配置")
        ...
        try:
            ...
            for target_path in target_paths:
                ...
                updated_count += 1
                self.log_operation(f"成功: 已将 {source_name} 的Default.save复制到 {self.folders[target_path]}")
            
            QMessageBox.information(self, "成功", 
                f"已成功将 {source_name} 的Default.save复制到 {updated_count} 个配置")
    ...
    def execute_function4(self):
        ...
        if len(self.target_option_combo.checkedItems()) == 0:
            QMessageBox.warning(self, "警告", "请至少选择一个目标配置!")
            return
        ...
        # 获取选中的目标配置
        target_paths = self.target_option_combo.checkedData()
        
        self.log_operation(f"执行: 从配置 {source_name} 复制选定选项到选中的配置")
        ...
        try:
            ...
            for target_path in target_paths:
                target_file = os.path.join(target_path, "Default.save")
                if not os.path.exists(target_file):
                    self.log_operation(f"跳过: {self.folders[target_path]} 没有Default.save文件")
                    continue
                ...
                for field, field_name in fields_to_copy:
                    if field in source_data:
                        ...
                        self.log_operation(f"已更新: {self.folders[target_path]} 的 {field_name} 配置")
                        updated = True
            ...
            QMessageBox.information(
                self, "完成", 
                f"已成功更新 {updated_count}/{len(target_paths)} 个配置的选定选项"
            )
